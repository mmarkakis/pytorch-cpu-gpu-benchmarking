...
for input_dim in dimensionality_list:
    cpu_time = train_model(
        torch.device("cpu"), input_dim, num_points_fixpoint, num_epochs_fixpoint
    )
    cpu_times_dims.append(cpu_time)

    if torch.cuda.is_available():
        gpu_time = train_model(
            torch.device("cuda"), input_dim, num_points_fixpoint, num_epochs_fixpoint
        )
    else:
...
for num_points in num_points_list:
    cpu_time = train_model(
        torch.device("cpu"), dimensionality_fixpoint, num_points, num_epochs_fixpoint
    )
...
for num_epochs in num_epochs_list:
    cpu_time = train_model(
        torch.device("cpu"), dimensionality_fixpoint, num_points_fixpoint, num_epochs
    )
...
plt.title(
    ...
    f"""({num_points_fixpoint} points, {num_epochs_fixpoint} epochs)"""
)
plt.legend()
...
plt.savefig(f"results/{exptime}/plot.png")
